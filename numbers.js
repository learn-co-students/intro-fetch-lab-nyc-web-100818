...
function getAnyNumberTrivia(triviaForm) {
  ...
  // validation
  if (isNaN(input)) {
    triviaDiv.innerHTML = "please enter a valid number";
  } else {
...
function showAllTheNumbers() {
  ...
  fetch('http://numbersapi.com/1..100').then(r => r.json())
    .then(allNumbersData => {
      let html = '<ul>'
      for (key in allNumbersData) {
        html += `<li>${allNumbersData[key]}</li>`
      }
      html += '</ul>'
      allNumbersDiv.innerHTML = html
    })
}
